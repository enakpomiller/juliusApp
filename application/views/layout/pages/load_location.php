                    ...
                    <?php  foreach ($get_prodby_location as $prod) {?>
                            <section class="col-lg-3 col-md-6 py-3">
                                        <div class="card" id="prod-cover">
                                            <div class="card-body">
                                            <a href="<?=base_url('home/seller_buy/'.$prod->prod_id)?>"> <img   src="<?=base_url('assets/sellers_uploads/'.json_decode($prod->file_name))?>" class="w-100" style="height:300px;"></a>
                                            </div>
                                            <h5 class="text-center"> <?=$this->db->get_where('tbl_sellers_products',array('sell_prod_id'=>$prod->prod_id))->row()->prod_name?>  </h5>
                                            <h5 class="text-center"> <?=number_format(($this->db->get_where('tbl_sellers_products',array('sell_prod_id'=>$prod->prod_id))->row()->prod_price ),2)?>  </h5>
